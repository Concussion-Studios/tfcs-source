...
void IN_SaveMeDown(const CCommand &args) { KeyDown(&in_saveme, args[1]); }
void IN_SaveMeUp(const CCommand &args) { KeyUp(&in_saveme, args[1]); }
...
int CInput::GetButtonBits( int bResetState )
{
	...
	CalcButtonBits(bits, IN_SAVEME, s_ClearInputState, &in_saveme, bResetState);
...
static ConCommand startSaveMe("+saveme", IN_SaveMeDown);
static ConCommand endSaveMe("-saveme", IN_SaveMeUp);
