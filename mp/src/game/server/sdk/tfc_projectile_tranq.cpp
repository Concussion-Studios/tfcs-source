...
//
// Purpose: projectile shooted from the tranquilizer
//
//=============================================================================//
#include "cbase.h"
...
BEGIN_DATADESC( CTFCProjectileTranq )
	DEFINE_ENTITYFUNC( ProjectileTouch ),
END_DATADESC()

LINK_ENTITY_TO_CLASS( tfc_projectile_tranq, CTFCProjectileTranq );
PRECACHE_REGISTER( tfc_projectile_tranq );

CTFCProjectileTranq::CTFCProjectileTranq()
{
}
...
void CTFCProjectileTranq::Precache( void )
{
	PrecacheModel( PROJWCTILW_NAIL_MODEL );
}

void CTFCProjectileTranq::Spawn()
{
	Precache( );
...
void CTFCProjectileTranq::ProjectileStickTo( CBaseEntity *pOther, trace_t &tr )
{
...
void CTFCProjectileTranq::ProjectileTouch( CBaseEntity* pOther )
{
	...
	if ( FClassnameIs( pOther, "tfc_projectile_tranq" ) )
		return;
...
CTFCProjectileTranq *CTFCProjectileTranq::CreateTranq(
	...
	float flDamage )
{
	auto* pTranq = static_cast< CTFCProjectileTranq* >( CreateEntityByName( "tfc_projectile_tranq" ) );
	if ( !pTranq )
	{
		Assert( pTranq );
		return NULL;
	...
	pTranq->SetOwnerEntity( pOwner );
	pTranq->Spawn();
	pTranq->SetAbsVelocity( vecVelocity );
	pTranq->SetAbsAngles( vecAngles );
	pTranq->SetAbsOrigin( vecOrigin );
	pTranq->ChangeTeam( pOwner->GetTeamNumber() );
	pTranq->SetDamageAmount( flDamage );
